...
Info
    Defines WaitCommandParser parsing WAIT statements.
'''
...
class WaitCommandParser:
    '''
        Parses WAIT statements into AST instruction nodes.

        It defines:

            :attributes:
                | COMMAND_NAME - Command keyword identifier string ('WAIT').
                | PARAM_SECONDS - Parameter key for wait duration in seconds ('seconds').
            :methods:
                | can_parse - Checks if command is WAIT.
                | parse - Parses WAIT tokens.
                | get_version - Returns parser version string.
    '''

    COMMAND_NAME: ClassVar[str] = 'WAIT'
    PARAM_SECONDS: ClassVar[str] = 'seconds'

    def can_parse(self, command_name: str) -> bool:
        '''
            Checks if command is WAIT.

            :param command_name: Command keyword string.
            :return: True if command is WAIT, False otherwise.
            :exceptions: None.
        '''
    ...
    def parse(
        ...
        '''
            Parses WAIT tokens into MyCobotInstruction.
            ...
            :exceptions:
                | ValueError: Invalid syntax or missing duration value.
        '''
        if len(tokens) < 2 or tokens[1].token_type != MyCobotTokenType.NUMBER:
            raise ValueError(
                f'Line {line_number}: WAIT command expects duration in seconds.'
            )

        duration: float = float(tokens[1].value)

        return MyCobotInstruction(
            command_type=MyCobotCommandType.WAIT,
            line_number=line_number,
            parameters={self.PARAM_SECONDS: duration},
            raw_text=raw_text
        )
