...
Info
    Defines HomeCommandParser parsing HOME statements.
'''
...
class HomeCommandParser:
    '''
        Parses HOME statements into AST instruction nodes.

        It defines:

            :attributes:
                | COMMAND_NAME - Command keyword identifier string ('HOME').
            :methods:
                | can_parse - Checks if command is HOME.
                | parse - Parses HOME tokens.
                | get_version - Returns parser version string.
    '''

    COMMAND_NAME: ClassVar[str] = 'HOME'

    def can_parse(self, command_name: str) -> bool:
        '''
            Checks if command is HOME.

            :param command_name: Command keyword string.
            :return: True if command is HOME, False otherwise.
            :exceptions: None.
        '''
    ...
    def parse(
        ...
        '''
            Parses HOME tokens into MyCobotInstruction.
