...
Info
    Defines RelaxCommandParser parsing RELAX statements.
'''
...
class RelaxCommandParser:
    '''
        Parses RELAX statements into AST instruction nodes.

        It defines:

            :attributes:
                | COMMAND_NAME - Command keyword identifier string ('RELAX').
            :methods:
                | can_parse - Checks if command is RELAX.
                | parse - Parses RELAX tokens.
    '''

    COMMAND_NAME: ClassVar[str] = 'RELAX'

    def can_parse(self, command_name: str) -> bool:
        '''
            Checks if command is RELAX.

            :param command_name: Command keyword string.
            :return: True if command is RELAX, False otherwise.
            :exceptions: None.
        '''
    ...
    def parse(
        ...
        '''
            Parses RELAX tokens into MyCobotInstruction.
