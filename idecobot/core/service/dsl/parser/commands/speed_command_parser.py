...
Info
    Defines SpeedCommandParser parsing SPEED statements.
'''
...
class SpeedCommandParser:
    '''
        Parses SPEED statements into AST instruction nodes.

        It defines:

            :attributes:
                | COMMAND_NAME - Command keyword identifier string ('SPEED').
                | PARAM_VALUE - Parameter key for numerical speed value ('value').
            :methods:
    ...
    PARAM_VALUE: ClassVar[str] = 'value'
    ...
    def can_parse(self, command_name: str) -> bool:
        '''
            ...
            :return: True if command is SPEED, False otherwise.
            :exceptions: None.
        '''
    ...
    def parse(
        ...
        '''
            Parses SPEED tokens into MyCobotInstruction.
            ...
            :exceptions:
                | ValueError: Invalid syntax or missing speed value.
        '''
        if len(tokens) < 2 or tokens[1].token_type != MyCobotTokenType.NUMBER:
            raise ValueError(
                f'Line {line_number}: SPEED command expects a numerical percentage value.'
            )

        speed_val: float = float(tokens[1].value)
